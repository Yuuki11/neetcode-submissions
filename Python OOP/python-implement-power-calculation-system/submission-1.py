class Hero:
    ...
    @staticmethod
    def calculate_effective_power(base_power: int, attributes: {strength: int, speed: int, intelligence: int}) -> int:
        attribute_bonus = sum(attributes.values
        ()) / len(attributes)
        effective_power = base_power * (1 + attribute_bonus)
        final_result = round(effective_power, 1)
        return final_result        
